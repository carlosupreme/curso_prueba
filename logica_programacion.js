...
function recortarCadena(string, stop) {
  for (let i in string) {
    if (i == 4) break;
    str += string[i];
  }
...
function isPalindromo(string) {
  ...
  return `El texto "${string}" ${
    string.toLowerCase() === validacion ? "si" : "no"
  } es un palindromo`;
}
...
console.log(
  "8.1:",
  removePatternWithRegExp("xyz1, xyz2, xyz3, xyz4 y xyz5", "[xyz]")
);
...
function isCapicua(integer) {
  if (!integer || typeof integer !== "number") return alerta;
  let volteado = integer.toString().split("").reverse().join("");
  return `El numero "${integer}" ${
    integer.toString() === volteado ? "si" : "no"
  } es capicua`;
}
console.log("10:", isCapicua(2002));
// 11) Programa una función que calcule el factorial de un número (El factorial de un entero positivo n, se define como el producto de todos los números enteros positivos desde 1 hasta n), pe. miFuncion(5) devolverá 120
function factorial(n) {
  if (!n || n < 0 || Number.isInteger(n) === false || typeof n !== "number")
    return alerta;
  let contador = 1;
  for (let i = 1; i <= n; i++) {
    contador *= i;
...
function isOddOrEven(n) {
  if (!n || Number.isInteger(n) === false || typeof n !== "number")
    return alerta;
...
function diasPasados(date) {
  ...
  if (new Date(date).getFullYear() > fechaHoy.getFullYear())
    return "Ese año aun no existe";
  if (typeof date === "string") {
    let msPasados = Math.floor(fechaHoy - new Date(date));
    return `Han pasado ${Math.floor(msPasados / 31556952000)} años`;
  }
  if (date instanceof Date === true)
    return `Han pasado ${Math.floor(
      Math.floor(fechaHoy - date) / 31556952000
    )} años`;
  return alerta;
}
...
function validarNombre(string) {
  ...
  return `El nombre "${string}" ${
    /^[A-Za-zÁÉÍÓÚÜáéíóúü\s]+$/g.test(string) ? "si" : "no"
  } es válido`;
}
...
function elevarAlCuadrado(array) {
  if (!array || array.length < 1 || array instanceof Array === false)
    return alerta;
...
function elevarAlCuadrado2(array) {
  if (!array || array.length < 1 || array instanceof Array !== true)
    return alerta;
  for (let el of array) if (typeof el !== "number") return alerta;
...
function maximosMinimos2(array) {
  if (!array || array.length < 1 || array instanceof Array !== true)
    return alerta;
  for (let el of array) if (typeof el !== "number") return alerta;
  return `Su maximo es: ${Math.max(...array)} y su minimo es: ${Math.min(
    ...array
  )}`;
}
...
function eliminandoDuplicados(array) {
  if (!array || array.length < 2 || array instanceof Array !== true)
    return alerta;
  return {
    array,
...
console.log(
  "25:",
  eliminandoDuplicados(["x", 10, "x", 2, "10", 10, true, true])
);
// another way
...
console.log(
  "25.1:",
  eliminandoDuplicados2(["x", 10, "x", 2, "10", 10, true, true])
);
...
class Pelicula {
  ...
  validarId(id) {
    ...
    else if (!/^([a-z]){2}([0-9]){7}$/.test(id))
      return console.warn(`El id: ${id} no cumple con los estandares IMDB`);
  }
  validarTitulo(titulo) {
    if (!this.validarStrings(titulo))
      return console.warn("Titulo no valido porque no es string");
    if (!this.validarNumCaracteres(titulo, 100))
      return console.warn("Longitud de titulo incorrecto");
  }
  validarDirector(director) {
    if (!this.validarStrings(director))
      return console.warn("Director no valido porque no es string");
    if (!this.validarNumCaracteres(director, 50))
      return console.warn("Longitud de director incorrecto");
  }
  validarYear(year) {
  ...
  validarPaises(paises) {
    if (!this.validarArrays(paises))
      return console.warn("Paises Debe ser un arreglo con datos validos");
  }
  validarGeneros(generos) {
    if (!this.validarArrays(generos))
      return console.warn("Los generos deben ser un arreglo con datos validos");
    else {
      for (let genre of generos) {
        if (!Pelicula.listaGeneros.includes(genre))
          return console.warn("Un genero no existe");
      }
    }
  }
  ...
  validarCalificacion(calificacion) {
    if (
      ...
      return console.warn(
        "La calificacion debe ser un numero entre 0-10 y con un decimal"
      );
